...
| `@deepseek-ai/dsh-tool-session-tree` | `session_tree` | `ctx.tools`、`ctx.systemPrompt`、`ctx.commands`、`ctx.agents`、`ctx.sessionTree` | `tool/call`、`session-tree/* events or an official replace surface event`、`tool/result` | - | session_tree projects the durable Harness Session log into a Pi-style immutable tree; jump/fork select the same model-visible message path without deleting sibling history. |
...
<a id="deepseek-aidsh-tool-session-tree"></a>

## `@deepseek-ai/dsh-tool-session-tree`
...
Source: [`packages/extensions/tool-session-tree/src/index.ts`](../packages/extensions/tool-session-tree/src/index.ts)
