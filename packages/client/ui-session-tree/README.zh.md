...
- 输入 `/tree` 打开或刷新右侧会话树。
- `/fork` 只展示可选择的用户消息节点，并在选中后创建当前项目下的独立会话副本；`/clone` 直接把当前会话完整复制到同项目的新会话，无需先选节点。
...
（`session-tree-branches`）会在侧边栏会话列表旁渲染一个左停靠的分支栏：
每个分叉子会话直接展示在原父会话下方，带缩进、纵向连接轨道、转肘接点
和展开/收起控件，并显示播种提示词的简短摘要。分叉落地时分支栏自动展开，
并跟随响应式会话列表更新——新分支出现无需刷新浏览器。

实现上只做表现层：分叉数据与父子关系完全来自官方原生分叉 API
（`ctx.sessions.fork` 会以 `parentId` 写入子会话），分支栏的级联结构纯粹
从该响应式列表推导。一个很小的 UI 层标注注册表（`fork-lineage.ts`，持久化
到 `localStorage`）只负责标记哪些子会话来自 `/fork`——因此 `/clone` 会话与
普通会话不会进入分支栏，保持原有侧边栏展示不变。`sidebar` 槽位本身保持
